...
def findWeight(weightName, weights):
    for e in weights:
        if(e.get_name() == weightName):
            return e.get_value()
    return 0
...
class Neuron(DifferentiableElement):
    ...
    def compute_output(self):
        # self.get_inputs[0].get_descendant_weights
        neuronValue = 0.0
        inputs = self.my_inputs
        for e in inputs:
            if(e.get_name()[0] == 'i' and e.get_name()[1] == '0'):
                neuronValue += (e.output() * findWeight(('w' + self.my_name), self.my_weights))
            elif(e.get_name()[0] == 'i'):
                neuronValue += (e.output() * findWeight(('w' + e.get_name()[1:] + self.my_name), self.my_weights))
            else:
                neuronValue += (e.output() * findWeight(('w' + e.get_name() + self.my_name), self.my_weights))
        return ((1 + math.exp(-1 * neuronValue)) ** -1);
        """
...
class PerformanceElem(DifferentiableElement):
    ...
    def dOutdX(self, elem):
        return (self.my_desired_val - self.output()) * (self.my_input.dOutdX(elem))
        """
