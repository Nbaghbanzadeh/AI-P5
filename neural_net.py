...
class RegularizedPerformanceElem(PerformanceElem):
    def output(self):
        return -1*((1/2) * ((self.my_desired_val - self.my_input.output())**2)) - (0.009*self.my_norm)
        raise NotImplementedError("Implement me!")

    def dOutdX(self, elem):
        return (self.my_desired_val - self.my_input.output()) * (self.my_input.dOutdX(elem)) - (0.009*self.my_dNorm)
        raise NotImplementedError("Implement me!")
        
    def set_norm(self, norm):
        self.my_norm = norm
    def set_dNorm(self, dNorm):
        self.my_dNorm = dNorm
...
def train(network,
    ...
    while iteration < max_iterations:
        ...
        for datum in data:
            ...
            dNorm = findDnorm(allWeights)
            network.performance.set_norm(norm)
            network.performance.set_dNorm(dNorm)
            #'''
